...
float distanceToLine(const lineParam& lp, const position & pos, const bool outSideSegMeas = true) {
  const float x = pos[0];
  const float y = pos[1];
  const float z = pos[2];
...
float intersectionPlaneAndLine (const lineParam& plane, const lineParam& lp2) {
...
float distanceBetweenTwoLines(const lineParam & lp1, const lineParam & lp2) {
...
void addLines(std::vector<lineParam>& lineParams, const float a[3], const float b[3], const float c[3], const float d[3], const float sampleDistance) {
...
void pathToSpline(  const centerlinePoints & inputpath,
