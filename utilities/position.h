...
position operator* (float lhs, const position &rhs) {
  position temp;
  temp[0] = rhs[0]*lhs;
  temp[1] = rhs[1]*lhs;
  temp[2] = rhs[2]*lhs;
...
position operator* (double lhs, const position &rhs) {
  position temp;
  temp[0] = rhs[0]*float(lhs);
  temp[1] = rhs[1]*float(lhs);
  temp[2] = rhs[2]*float(lhs);
  temp[3] = rhs[3]*float(lhs);
  return temp;
}
