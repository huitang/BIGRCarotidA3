...
Path::PathType Path::orderVoxels (const PathType &input) {
...
double Path::computePathLength (const PathType & list) {
  double length = 0.0;
...
void Path::smoothAndResamplePath (PathType & path, PathType & outputpath, const float sigma=1.0f, const float precision=5.0f, const float sampleDistance=1.0f, const bool orderMarkers=true) {
  ...
  if (nrVoxels > 3) {
    ...
    // input markers)
    std::vector<SplineFitter::lineParam> centerSplineLines;
    SplineFitter::pathToSpline (smoothedMarkers, sampleDistance, centerSplineLines);

    // Sample with equidistance along the line segments on the spline
    SplineFitter::centerlineSpline equiDistSpline;
    SplineFitter::sampleEquiDistant (centerSplineLines, sampleDistance, equiDistSpline);

    // Output a path at the start of the equidistance sampled line segments
    SplineFitter::splineToPath (equiDistSpline, sampleDistance, outputpath);
  }
}

void Path::smoothPath( const PathType & inputpath, 
                 PathType & outputpath,
                 const float sigma,
