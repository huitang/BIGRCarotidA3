#ifndef __MEDIALNESSIMAGEFILTERNEW__TXX
#define __MEDIALNESSIMAGEFILTERNEW__TXX
...
::ThreadedGenerateData(const typename ImageToImageFilter<TInputImage,TOutputImage>::OutputImageRegionType& outputRegionForThread, int threadId)
{
  ...
  faceListX = faceCalculatorX( m_XGradientImage,
  ...
  faceListY = faceCalculatorY( m_YGradientImage,
                              outputRegionForThread,
                              radius );
  ...
  for ( fitX=faceListX.begin(), fitY=faceListY.begin(); fitX != faceListX.end(); ++fitX, ++fitY ){
    ShapedNeighborhoodIteratorType gradientImageXIt(radius, m_XGradientImage, *fitX);
    ShapedNeighborhoodIteratorType gradientImageYIt(radius, m_YGradientImage, *fitY);
    ImageIterator outputIt(this->GetOutput(),*fitX);
    ImageIterator maskIt;
    if ( m_UseMask && this->GetMaskImage() ){
      maskIt = ImageIterator(this->GetMaskImage(),*fitX);
      maskIt.GoToBegin();
    }
    ...
    for (int i=0; i<this->m_NumberOfAngles;i++)
    {
      ...
      { 
        ...
        gradientImageXIt.ActivateOffset(off);
        gradientImageYIt.ActivateOffset(off);
      }
    }
    ...
    for (; !outputIt.IsAtEnd(); ++gradientImageXIt, gradientImageYIt, ++outputIt ){
      ...
      if ( useCurrentVoxel ){
        ShapedNeighborhoodIteratorType::ConstIterator ciX = gradientImageXIt.Begin();
        ShapedNeighborhoodIteratorType::ConstIterator ciY = gradientImageYIt.Begin();
        for (; ciX.IsAtEnd(); ++ciX, ++ciY ){
          ...
          const float graX = ciX.Get();
          const float graY = ciY.Get();
...
#endif //__MEDIALNESSIMAGEFILTERNEW__TXX
